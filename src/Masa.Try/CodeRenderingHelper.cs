...
public class CodeRenderingHelper
{
    ...
    public static Type? RenderingToType(RenderingToTypeOptions options)
    {
        ...
        return assembly.GetType("Masa." + options.ComponentName);
    }
    ...
    /// <param name="ignoreNames">忽略加载的程序集名称</param>
    /// <returns></returns>
    public static async Task InitializedAsync(List<PortableExecutableReference>? refs, List<RazorExtension> extensions)
    {
        _refs = refs;
        exts = extensions;
    }
